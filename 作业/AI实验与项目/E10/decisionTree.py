...
姓名：孙新梦
学号：18340149
TASK:使用ID3算法实现决策树，判断一个人是否每年能拿到超过5万
    输入：两个文件：
    adult.data：训练集
    adult.test：测试集
    输出：10个测试样例的精确度，最后的平均精确度
    运行：
    python decisionTree.py  #普通版
    python decisionTree.py --post_proning 1 #后剪枝
    python decisionTree.py --print_tree 1 #打印决策树到文件中
    python decisionTree.py --ignore 1 #忽略掉第7，12，13特征
"""
import numpy as np
import random
import copy
...
class Tree(object):
    ...
    def buildNode(self, now_root):
        ...
        for i in range(self.feature_length):
            ...
            now_gain = (1 - (len(miss_index) / len_data)) * self.getGainRadio(now_root, now_sub_data_indexs, i,
                                                                              now_entropy)
    ...
    def predictNode(self, now_node, test_feature):
        ...
        for i in range(len(now_node.son)):
            if self.isContinuous(now_divide_feature_index):
                ...
                elif i == len(now_node.son) - 1 and test_feature[now_divide_feature_index] >= now_node.son[
                    i].divide_value:
                    return self.predictNode(now_node.son[i], test_feature)
            else:
...
if __name__ == '__main__':
    ...
    # if args.best_parameter == 1:
        best_max_continous_son, best_max_leaf_length = getBestParameter(train_data, train_label, test_data, test_label,
                                                                        continous_list)
